...
"""%s
 ...
 - author      : SUFYAN KHAN
 - facebook    : facebook.com/100073654452342
 - fanspage    : facebook.com /groups/230978421837448
 - whatsapp     : 03305853769
 - github      :   https://github.com/khan001/sufyankh045
 - script name : khan
 - version     : 1.2
...
_  ___   _    _    _   _
| |/ / | | |  / \  | \ | |%s> %s CREATE BY KHAN 
| ' /| |_| | / _ \ |  \| |%s>%s  FULL NAME  SUFYAN
| . \|  _  |/ ___ \| |\  |%s>%s  PATHAN
|_|\_\_| |_/_/   \_\_| \_|%s>%s PAKISTAN   
 %s[%s*%s] By : %sSUFYAN KHAN
 %s[%s*%s] --------------------------------------
 [%s*%s] IP : %s%s
...
def masuk():
    ...
    print ('\n%s [1] Login in fb token \n [2] how to get  token \n [%s3%s] Exit'%(P,M,P))
    rom = raw_input('\n%s [?]  select Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [! ]correct content kentod "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] please do not use main account  create new facebook account and and use"%(P,M,P))
    	romz = raw_input('%s [?] facebook Token : %s'%(P,K))
        if romz in(""):
        	print("%s [!] correct content kentod "%(M));exit()
    	try:
        ...
        print   ('\n%s[01] https://youtu.be/U865VYYDp0k \n [2] https://bit.ly/3C2mXtg \n [%s3%s] back'%(P,M,P))  
